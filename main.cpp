...
BezierCurve* bCurveSimple;
BezierCurve* bCurveSecond;
BezierCurveMesh* bCurveMeshSimple;
BezierCurveMesh* bCurveMeshSecond;
...
int main()
{
	...
	// Bezier CURVE
	bCurveSimple = new BezierCurve(nbLine, controlPoints8_3D);
    bCurveSecond = new BezierCurve(offSetLine, controlPoints8_3D, true);
	...
	// CURVE MESH
    bCurveMeshSimple = new BezierCurveMesh(bCurveSimple->getCurve(), bCurveSimple->getIndices(), &camera);
    bCurveMeshSecond = new BezierCurveMesh(bCurveSecond->getCurve(), bCurveSecond->getIndices(), &camera);
    controlCurveMesh = new ControlCurveMesh(controlPoints8_3D, &camera);
	...
	//CURVE
    bCurveMeshSimple->setupMesh();
    bCurveMeshSecond->setupMesh();
    controlCurveMesh->setupMesh();

    // Shaders instanciation
    ShaderProgram* shaderLights = new ShaderProgram("vertexLight.glsl", "fragmentLight.glsl");

	ShaderProgram* shaderCurve = new ShaderProgram("vertexShader.glsl", "fragmentCurve.glsl");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (showCurve == 0 || showCurve == 1) {
            ...
            if (showBezier)
				if (showCurve == 0)
                    bCurveMeshSimple->draw(shaderCurve, (float)SCR_WIDTH, (float)SCR_HEIGHT);
                else
                    bCurveMeshSecond->draw(shaderCurve, (float)SCR_WIDTH, (float)SCR_HEIGHT);
        }
        else {
            //Polynome de control
            if (showParam)
    ...
    //CURVE
    bCurveSimple->deleteCurve();
    bCurveSecond->deleteCurve();
    delete bCurveSimple;
    delete bCurveSecond;
    delete bCurveMeshSimple;
    delete bCurveMeshSecond;	
    delete controlCurveMesh;
    delete shaderCurve;
...
void updateCurve() {
    ...
    if (showCurve == 0) {
        ...
        bCurveSimple -> changeCurve(nbLine);
        bCurveMeshSimple -> setVertices(bCurveSimple->getCurve());
        bCurveMeshSimple -> setIndices(bCurveSimple->getIndices());
        bCurveMeshSimple->setupMesh();
    }
    else {
        cout << "offSetLine = " << offSetLine << endl;
        bCurveSecond->changeCurve(offSetLine);
        bCurveMeshSecond->setVertices(bCurveSecond->getCurve());
        bCurveMeshSecond->setIndices(bCurveSecond->getIndices());
        bCurveMeshSecond->setupMesh();
    }
}
...
void controlChange() {
    if (showCurve == 0 || showCurve == 1) {
        ...
        bCurveSimple->setControlPoints(controlPoints, nbLine);
        bCurveMeshSimple->setVertices(bCurveSimple->getCurve());
        bCurveMeshSimple->setIndices(bCurveSimple->getIndices());
        bCurveMeshSimple->setupMesh();
			
        bCurveSecond->setControlPoints(controlPoints, offSetLine);
        bCurveMeshSecond->setVertices(bCurveSecond->getCurve());
        bCurveMeshSecond->setIndices(bCurveSecond->getIndices());
        bCurveMeshSecond->setupMesh();
    }
